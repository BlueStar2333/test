      ...
      <el-tab-pane label="组件设置" name="first">
        ...
        >
          ...
          <div v-show="radioShow" class="setting">
            ...
            <div
              v-for="(item, index) in componentData.content"
              :key="index"
              class="setting-main"
            >
              <el-radio
                v-show="componentData.type === 2"
                v-model="componentData.select"
                :label="item"
              />
              <el-checkbox
                v-show="componentData.type === 3"
                v-model="componentData.select"
                :label="item"
              />
              <el-input
                v-model="componentData.content[index]"
                size="mini"
                placeholder="请输入内容"
                style="width: 140px"
              />
            ...
            <div class="setting-link">
              <el-link
                ...
                type="primary"
                @click="
                  componentData.content.push(
                    `选项${componentData.content.length + 1}`
                  )
                "
                >增加选项</el-link
              >
      ...
      <el-tab-pane label="表单设置" name="second">
        <el-form ref="form" class="form-setting" label-width="80px" size="mini">
          ...
          <el-form-item label="表说明">
            <el-input
              v-model="diyForm.diyDescription"
              type="textarea"
              autosize
              placeholder="请输入此表的描述,目的等"
            />
          </el-form-item>
          <el-form-item label="起始日期">
            <el-date-picker
              v-model="diyForm.diyStartDate"
              type="date"
              placeholder="选择日期"
              format="yyyy-MM-dd"
...
export default {
  ...
  destroyed() {
    this.$eventBus.$off("selectInput");
    this.$eventBus.$off("getDiyData");
  ...
  methods: {
    init() {
      ...
      getUserinfoByName(data).then((res) => {
        this.options = JSON.parse(res.d);
        // console.log(this.options, 2567)
      });
      if (this.editForm) {
        this.diyForm = {
        ...
        this.operator = JSON.parse(this.editForm.diyOperator);
      }
    },
    changeTab(tab) {
      this.activeName = tab;
    ...
    getDiyData(cb) {
      this.diyForm.diyOperator = JSON.stringify(this.operator);
      cb(this.diyForm);
    },
    deleteRadio(index) {
