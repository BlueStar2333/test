...
const CoSaveDiyForm = (req, res) => {
  ...
  pool.query(sql, [creator, table_name, description, reqContent, verify_correct, check_number, new Date()], (error, result) => {
...
const CoEditDiyForm = (req, res) => {
  ...
  pool.query(sql, [creator, table_name, description, reqContent, check_number, verify_correct, new Date(), id], (error, result2) => {
    if (error) throw error;
    $api.ReturnJson(res, {
      code: 1,
      msg: "修改成功",
