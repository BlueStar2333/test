...
const VaSaveDiyForm = (req, res, next) => {
  // POST接收参数
  $api.PostArg(req).then(({ creator, table_name, description, content, verify_correct, check_number }) => {
      req.validData = $api.SaveValidData(req.validData, { creator, table_name, description, content, verify_correct, check_number }); // 保存校验对象
      next(); // 校验通过
  });
};
